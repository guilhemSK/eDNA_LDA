...
# Code written by Guilhem Sommeria-Klein, feb. 2017

# This code calls the following functions stored in the "code_insert" directory, 
# depending on the options chosen in LDA_parameters_file.R :
...
source(paste0(code_insert,parameterFile))  
...
library(rgdal,lib.loc=lib_path)
library(sp)
library(rgeos,lib.loc=lib_path)
library(maptools)
...
if (length(which(c(barcode_gh,barcode_ghassigned,barcode_itsfungi,barcode_itsasco,barcode_itsbasidio,barcode_itsglomero,bacteriafungiarchaea,barcode_16sbact,
                   ...
                   frogs_iucn_open,frogs_poly1_all,frogs_poly1_forest,frogs_poly1_open,frogs_poly2_all,frogs_poly2_forest,frogs_poly2_open,
                   barcode_planktonV9_byStation, barcode_planktonV9_all,barcode_planktonV9_CompleteSizeRange_byStationByDepth,
                   barcode_planktonV9_Surface_CompleteSizeRange_byStation,barcode_planktonV9_DCM_CompleteSizeRange_byStation)==1))>1)
...
{
  if (low_memory)
    source(paste0(code_insert,"LDA_realization_comparison_lowMemory_fun.R"))
  else
...
} else if (data_tara)
{
  ...
  if (barcode_planktonV9_CompleteSizeRange_byStationByDepth)
    SurDCMperStation = 1
  else if (barcode_planktonV9_all)
    UnlimitedPointsPerStation = 1
}
...
} else if (barcode_planktonV9_byStation)
{
  barcode_insert = "18S_V9_TARA_byStation"
  short_barcode_insert = "18SV9"
} else if (barcode_planktonV9_all)
{
  barcode_insert = "18S_V9_TARA"
  short_barcode_insert = "18SV9"
} else if (barcode_planktonV9_CompleteSizeRange_byStationByDepth)
{
  barcode_insert = paste0("18S_V9_TARA_CompleteSizeRange_byStationByDepth_",taxa_insert,noArcticNoBiomark_insert,noLagoon_insert)
} else if (barcode_planktonV9_Surface_CompleteSizeRange_byStation)
{
  barcode_insert = paste0("18S_V9_TARA_Surface_CompleteSizeRange_byStation_",taxa_insert,noArcticNoBiomark_insert,noLagoon_insert)
} else if (barcode_planktonV9_DCM_CompleteSizeRange_byStation)
{
  barcode_insert = paste0("18S_V9_TARA_DCM_CompleteSizeRange_byStation_",taxa_insert,noArcticNoBiomark_insert,noLagoon_insert)
}
...
} else
{
  ...
  } else 
  {
    if (data_tara)
    {
      ...
      # coordTara has nrow equal to ncol(data2m), and has 0 Lat and Long for station TARA_017 (samples TV9_561 and TV9_562) 
      # and for the first 183 samples of data2m (samples BV9_1 to BV9_40 and TA9_1 to TA9_143 (TARA arctic))
      load("coordTara.Rdata")
    } else  
    {
...
if (mode(data2m) != "numeric")
  stop("The data2m matrix is not numeric.")
...
# Defining coordTara, coordGS and Missing_positions_indices to pass them to functions
if (!exists("coordTara"))
  coordTara = NULL
if (!exists("coordGS"))
  coordGS = NULL
if (!exists("Missing_positions_indices"))
  Missing_positions_indices = NULL
...
# Saving useful properties of the original data and removing them from memory
nrow_data2m = nrow(data2m)
ncol_data2m = ncol(data2m)
sum_data2m = sum(data2m)
colnames_data2m = colnames(data2m)

#cat("Class of data2m is:",class(data2m),"\n")
...
} else if ((filled && data_pp) || (!data_pp && !data_h20))
{
...
if (mpar)
{
  ...
  if (mnb_topics)
  {
    ...
    {  
      ...
      cat("Loading LDA decomposition from file",filename,"\n")
      load(filename)
    }
...
if (local)
  cat("Results will be saved in file",filename,"\n")
...
###################################################
# Defining variables for the loop over mpar_range #
###################################################
    ...
    } else if (local)
    {
      ...
      while (length(discarded_real)>0)
      {
        ...
        { 
          ...
          control_LDA_VEM = list(estimate.alpha=TRUE, alpha=alpha, estimate.beta=TRUE,
                                 verbose = 1, prefix = tempfile(), save = 0, keep = 1,
                                 seed = SEED, nstart = length(discarded_real), best = 0,
                                 var = list(iter.max = 500, tol = var_tol),
                                 em = list(iter.max = 1000, tol = em_tol),
                                 initialize = "random")
  ...
  if (mpar)
  {
    if (Rtopicmodels_Gibbs)
    {
      ...
      for (j in 1:nb_real)
      {
        ...
        AIC1[j,par_index] = 2*(nb_topics*(nb_terms+nb_doc-2) - Result[[j]]@fitted[[1]]@loglikelihood)
        AIC2[j,par_index] = 2*(nb_words - Result[[j]]@fitted[[1]]@loglikelihood)
        AIC3[j,par_index] = 2*(nb_topics*(nb_terms-1) + 1 - Result[[j]]@fitted[[1]]@loglikelihood)
        AICc[j,par_index] = 2*((nb_topics*(nb_terms-1) + 1)*(1+1/nb_doc) - Result[[j]]@fitted[[1]]@loglikelihood)
      } 
      LLH_final0[1,par_index] = mean(LLH_final1[,par_index])
      LLH_final0[2,par_index] = sd(LLH_final1[,par_index])      
      AIC0[1,par_index] = 2*(nb_topics*(nb_terms+nb_doc-2) - LLH_final0[1,par_index])
      AIC0[3,par_index] = 2*(nb_words - LLH_final0[1,par_index])
      AIC0[5,par_index] = 2*(nb_topics*(nb_terms-1) + 1 - LLH_final0[1,par_index])
    ...
    } else if (Rtopicmodels_VEM)
    {
      ...
      for (j in 1:nb_real)
      {
        ...
        AIC1[j,par_index] = 2*(nb_topics*(nb_terms+nb_doc-2) - sum(Result[[j]]@loglikelihood))
...
{ 
  ...
  if ((testdata || data_betadiv_pooled) && realization_comparison)
    ...
    #########################################################
{
  ...
  # nrow(data2m) = nb_MOTUs et ncol(data2m) = nb_samples
  # nrow(documents) = nb_samples and ncol(documents) = nb_topics
...
############################
cat("Realization ",j_select,": Saving topic composition ...\n",sep="")
...
for (k in 1:nb_topics)
{
  ...
  for (j in 1:length(taxo_ref[1,]))
    topic_compo_taxo[[k]][,j+1]=as.character(taxo_ref[vect$ix+1,j])
}
#remove(taxo_ref)
...
if ((j_select != 1) && (testdata || data_betadiv_pooled) && realization_comparison)
  ...
  for (j_real in 1:(j_select-1))
  {
    ...
    for (k in 1:nb_topics)
    {
      if (bij)
      {
        ...
        Var_KL_topic_comparison_randomized_allRealPairs_byTopic[[k]][j_real,j_select] = Var_KL_topic_comparison_randomized[k]
...
################################################################################################################
# Correlation to bacterial assemblages to name the assemblages in case (nb_topics == 3) for Petit Plateau data #
...
if (j_select == 1)
{
  ...
  diversity[nb_topics+1] = nrow_data2m
...
if ((j_select == 1) && abiotic_variables)
{
  ...
  # documents_randomized and KL_documents_randomized are computed only for (case == case_list[1])
  # and then loaded as input variables for subsequent values of case
  documents_randomized = 0
  KL_documents_randomized = 0
  for (case in case_list)
  {
    # Call to the LDA_abiotic_variables_fun function
    LDA_abiotic_variables_result = LDA_abiotic_variables_fun(data_pp,testdata,data_gs,case,local_prefix,data_insert,coordGS,
                                                             filled,filled_with_gaps,Missing_positions_indices,
                                                             ncol_data2m,sum_data2m,pca_abiotic,nb_topics,nb_abiotic_rndzations,
                                                             documents,KL_documents,occurrence,case_list,documents_randomized,KL_documents_randomized)
    ...
    if (case == case_list[1]) 
    {
      documents_randomized = LDA_abiotic_variables_result$documents_randomized
      KL_documents_randomized = LDA_abiotic_variables_result$KL_documents_randomized
      # SKL_bacteria contains both the SKL value and the normalized ES value
      if (data_pp && (nb_topics == 3))
      {
  ...
  cat("Computing correlations between the best realization and abiotic variables: Done.\n")
  
  #############################################
  # Comparison with abiotic variables: output #
  #############################################
  
  cat("Outputting the comparison to abiotic variables ...\n")
  ...
  if (data_pp)
  {
    ...
    if (nb_topics == 3)
      ...
      save(SKL_bacteria,p_value_SKL_bacteria,file="SKL_samplewise_between_bacterial_topics_and_best_real.Rdata")
  } else if (data_gs)
...
if ((j_select == 1) && testdata && (nb_topics == true_nb_topics))
{
  ...
  LDA_testdata_result = LDA_testdata_fun(nb_rndzations_true_documents,samplewise,MOTUwise,ncol_data2m,
                                         nrow_data2m,nb_topics,KL_documents,documents,true_documents,KL_topic_compo)
...
{
  library(vegan)
  #cat("Topic comparison within the best realization ...\n")
  
  if (low_memory)
  {
    source(paste0(code_insert,"LDA_best_real_lowMemory_fun.R"))
    LDA_best_real_result = LDA_best_real_lowMemory_fun(nb_rndzations_best_real,nrow_data2m,sort_normal_topic,KL_documents,
                                                       KL_topic_compo,assemblage_names_vect,documents,topic_compo,sum_data2m)
  } else
  {
    source(paste0(code_insert,"LDA_best_real_fun.R"))
    LDA_best_real_result = LDA_best_real_fun(nb_rndzations_best_real,nrow_data2m,sort_normal_topic,KL_documents,
                                             KL_topic_compo,assemblage_names_vect,documents,topic_compo,sum_data2m)
  }
  ...
  cat("Topic comparison within the best realization: Done.\n")
  
  cat("Outputting topic comparison within the best realization ...\n")
...

{
  ...
  cat("Realization ",j_select,": Outputting topic taxonomic composition ...\n",sep="")
  ...
  pdf("Site-normalized_MOTU_composition_for_each_topic_20firstseq.pdf")
  par(cex.lab=1.5,cex.main=1.7,cex.axis=1.5,lwd=2)
  #par(mar=c(5.1,4.1,4.1,2.1)
  #bottom left top right
  par(mar=c(15.1,10.1,4.1,4.1))
  for (k in 1:nb_topics)
  {
    ...
    {
      for (i in 1:20)
        labels[i]=paste(topic_compo_taxo[[k0]][i,5],tail(strsplit(topic_compo_taxo[[k0]][i,4],"|")[[1]],n=1),topic_compo_taxo[[k0]][i,3],sep=" - ")
    } else
    {
      for (i in 1:20)
        labels[i]=paste(topic_compo_taxo[[k0]][i,11],topic_compo_taxo[[k0]][i,10],topic_compo_taxo[[k0]][i,2],sep=" - ")
    }
  ...
  pdf("Site-normalized_MOTU_composition_for_each_topic_20firstseq_noNumber.pdf")
  par(cex.lab=1.5,cex.main=1.7,cex.axis=1.5,lwd=2)
  #par(mar=c(5.1,4.1,4.1,2.1)
  #bottom left top right
  par(mar=c(15.1,10.1,4.1,4.1))
  for (k in 1:nb_topics)
  {
    ...
    {
      for (i in 1:20)
        labels[i]=paste(topic_compo_taxo[[k0]][i,5],tail(strsplit(topic_compo_taxo[[k0]][i,4],"|")[[1]],n=1),sep=" - ")
    } else
    {
      for (i in 1:20)
        labels[i]=paste(topic_compo_taxo[[k0]][i,11],topic_compo_taxo[[k0]][i,10],sep=" - ")
    }
    text(1:20, par("usr")[3], srt = 45, adj = c(1.05,1.75), labels = labels, xpd = TRUE, cex=1.1)
    if (k==1)
      title(ylab="Relative abundance in the\n 1st most abundant class")
  ...
  {
    k0 = rev(sort_normal_topic$ix)[k]
    Ordered_topic_compo_taxo[[k]] = topic_compo_taxo[[k0]][which(topic_compo_taxo[[k0]][,1]>1/sum_data2m),]
    ...
    } else if (nb_topics == 3 && data_pp)
    {
      write(paste0("\n%%%%%%%%%%%%%\nFor topic ",k," - ",assemblage_names_vect[k],
    ...
    if (data_tara)
    {
      for (i in 1:min(50,length(which(topic_compo_taxo[[k0]][,1]>1/sum_data2m))))
        write(paste(topic_compo_taxo[[k0]][i,1],"-",topic_compo_taxo[[k0]][i,5],"-",tail(strsplit(topic_compo_taxo[[k0]][i,4],":")[[1]],n=1),"-",topic_compo_taxo[[k0]][i,6]),file=Taxo_compo_file,append=T)
    } else
    {
      for (i in 1:min(50,length(which(topic_compo_taxo[[k0]][,1]>1/sum_data2m))))
        write(paste(topic_compo_taxo[[k0]][i,1],"-",topic_compo_taxo[[k0]][i,11],"-",topic_compo_taxo[[k0]][i,10],"-",topic_compo_taxo[[k0]][i,12]),file=Taxo_compo_file,append=T)
    }
  }
...

{
  ...
  if ((data_pp && !testdata) || data_h20 || data_gs || data_tara)
  {
    if ((kriging && any(kriged_real == j_select)))
    {
      cat("Realization ",j_select,": Plotting spatial composition maps ...\n",sep="") 
      
      source(paste0(code_insert,"LDA_spatial_maps_fun.R"))
      
      LDA_spatial_maps_result = LDA_spatial_maps_fun(j_select,kriged_real,nb_topics,data_pp,data_h20,data_tara,
                                                     data_gs,filled,filled_with_gaps,Missing_positions_indices,colnames_data2m,
                                                     documents,coordGS,coordTara,
                                                     local_prefix,data_insert,sort_normal_topic,subsubsubdirname,
                                                     SurDCMperStation, UnlimitedPointsPerStation)
      ...
      if (data_tara)
      {
        pdf("Topic_ordered_by_site-normalized_abundance_composition_maps_kriged_oneplot.pdf")
        print(tmp.one.plot)
        dev.off()
        
        pdf("Topic_ordered_by_site-normalized_abundance_composition_maps_kriged_oneplot_discrete.pdf")
        print(tmp.one.plot.discrete)
        dev.off()
      }
    }
    
    if ((!kriging || !any(kriged_real == j_select)) && !data_gs && !data_tara)
    {
  ...
  } else if (data_betadiv_pooled)
  {
    ...
    for (ii in 1:length(new_names)) 
      new_names_indices[ii] = which(colnames_data2m == new_names[ii])
...
cat("End of the loop over realizations.\n")
...
} else if (mpar)
{
  ...
  save(nb_topics_range,LLH_final0,AIC0,AICc0,AIC1,AIC2,AIC3,AICc,file="AIC-llh.Rdata")
  ...
  if (mnb_topics)
  {
    ...
    {
      #######
      plotname = paste("AIC_compare_nb_topics*(nb_terms-1)+1_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_nb_iter",nb_iter,".pdf",sep="")
      pdf(plotname)
      #par(mar=c(5.1,4.1,4.1,2.1))
      par(mar=c(5.1,5.1,4.1,2.1))
      ...
      #######
      plotname = paste("AIC_compare_nb_topics*(nb_terms-1)+1_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_nb_iter",nb_iter,"_allpoints.pdf",sep="")
      pdf(plotname)
      #par(mar=c(5.1,4.1,4.1,2.1))
      par(mar=c(5.1,5.1,4.1,2.1))
      ...
      #######
      plotname = paste("AICc_compare_nb_topics*(nb_terms-1)*(1+1:nb_docs)+1_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_nb_iter",nb_iter,".pdf",sep="")
      pdf(plotname)
      #par(mar=c(5.1,4.1,4.1,2.1))
      par(mar=c(5.1,5.1,4.1,2.1))
      
      errbar(nb_topics_range,AICc0[1,],AIC0[1,]+AIC0[2,],AIC0[1,]-AIC0[2,],ann=F,cex.axis=1.5,lwd=2,type="p",col="black")
      ...
      #######
      plotname = paste("AICc_compare_nb_topics*(nb_terms-1)*(1+1:nb_docs)+1_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_nb_iter",nb_iter,"_allpoints.pdf",sep="")
      pdf(plotname)
      #par(mar=c(5.1,4.1,4.1,2.1))
      par(mar=c(5.1,5.1,4.1,2.1))
      
      plot(nb_topics_range,AICc[1,],ann=F,cex.axis=1.5,lwd=2,type="p",col="black",ylim=range(AIC3))
      for (j in 1:nb_real)

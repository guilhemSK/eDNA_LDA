...
taxo_ref = read.table(paste(short_data_insert,"_",short_barcode_insert,"_taxo_ref_table_assignScore.txt",sep=""), sep=" ")
...
#Alternatively : data2m_num = sapply(data2[-1,],as.numeric) -> converts directly into a matrix of numeric values
...
{ 
  ...
  for (j_select in 1:length_selected_real)
  {
    ...
    if (j_select == 1)
      norm_documents_real1 = norm_documents
    ...
    if (j_select == 1)
      sort_normal_topic_real1 = sort_normal_topic
...
#   if (j_select == 1)
#   {
#     data_lidar = read.table("Lidar_locsites_E20_origin_mean_10_extended.csv",sep=";")
#     data_abio = matrix(nrow=6,ncol=length(data_lidar$V5[-1]))
#     rownames(data_abio) = c("topo","canop","light","wetness","slope","canop_dif")
#     
#     data_abio[1,] = as.vector(data_lidar$V5[-1])
#     data_abio[2,] = as.vector(data_lidar$V4[-1])
#     data_abio[3,] = as.vector(data_lidar$V6[-1])
#     data_abio[4,] = as.vector(data_lidar$V7[-1])
#     data_abio[5,] = as.vector(data_lidar$V8[-1])
#     data_abio[6,] = as.vector(data_lidar$V10[-1])
#     for (i in 1:29)
#     {
#       for (j in 1:39)
#       {
#         data_topo_m[i,j] = as.numeric(data_topo_v[(i-1)*39+j])
#         data_canop_m[i,j] = as.numeric(data_canop_v[(i-1)*39+j])
#         data_light_m[i,j] = as.numeric(data_light_v[(i-1)*39+j])
#         data_wetness_m[i,j] = as.numeric(data_wetness_v[(i-1)*39+j])
#         data_slope_m[i,j] = as.numeric(data_slope_v[(i-1)*39+j])
#         data_log_slope_m[i,j] = log(data_slope_m[i,j])
#         data_canop_dif_m[i,j] = as.numeric(data_canop_dif_v[(i-1)*39+j])
#       }
#     }
#     
#     Correlation = matrix(nrow=nb_topics,ncol=6)
#     for (k in 1:nb_topics)
#     {
#       for (k_abio in 1:6)
#       {
#         for (j in 1:length(data_lidar$V5[-1]))
#         {
#           Correlation[k,k_abio] = norm_documents[j,rev(sort_normal_topic_real1$ix)[k]]*data_abio[]/nb_doc
#           + Correlation[[j_select-1]][kreal1,krealj]
#         }
#       }
#     }
#   }
  ...
  if (j_select != 1)
  {
    cat("Computing correlations between realizations 1 and",Selected_real[j_select],"...\n")      
    ...
    for (k_real1 in 1:nb_topics)
    {
      for (k_realj in 1:nb_topics)
      {
        for (j in 1:nb_doc) 
        {
          Correlation[[j_select-1]][k_real1,k_realj] = norm_documents_real1[j,rev(sort_normal_topic_real1$ix)[k_real1]]*norm_documents[j,rev(sort_normal_topic$ix)[k_realj]]/nb_doc
            + Correlation[[j_select-1]][k_real1,k_realj]
        }
        mean1 = sum(norm_documents_real1[,rev(sort_normal_topic_real1$ix)[k_real1]])/nb_doc
        meanj = sum(norm_documents[,rev(sort_normal_topic$ix)[k_realj]])/nb_doc
        Correlation[[j_select-1]][k_real1,k_realj] = Correlation[[j_select-1]][k_real1,k_realj] - mean1 - meanj
        Correlation[[j_select-1]][k_real1,k_realj] = Correlation[[j_select-1]][k_real1,k_realj]/sqrt(sum(norm_documents_real1[,rev(sort_normal_topic_real1$ix)[k_real1]]^2)/nb_doc-mean1^2)
        Correlation[[j_select-1]][k_real1,k_realj] = Correlation[[j_select-1]][k_real1,k_realj]/sqrt(sum(norm_documents[,rev(sort_normal_topic$ix)[k_realj]]^2)/nb_doc-meanj^2)
      }    
    }
  }
...
#     last_element=length(col_vector_rgb[,1])
    ...
    {  
      ...
      color2D.matplot(spatial_topicmix,c(0,1),c(0,0),c(1,0),
                      extremes=NA,cellcolors=NA,show.legend=TRUE,nslices=10,xlab="Column",
                      ylab="Row",do.hex=FALSE,axes=TRUE,show.values=FALSE,vcol="white",vcex=1,
                      border="black",na.color="white",main=paste("Topic ",k," - ",rev(sort_prop_topic$ix)[k],sep=""))
    }
    dev.off()
...
pdf("Topic_correlation_between_best_real_and_following_reals.pdf")
# 2 topics :
#par(mfrow=c(2,1))
# 5 topics :
#par(mfrow=c(3,2))
...
for (j_select in 2:length_selected_real)
{
  ...
  color2D.matplot(Correlation[[j_select-1]],c(0,1),c(0,0),c(1,0),

...
LDA_best_real_lowMemory_fun <- function(nb_rndzations_best_real,nrow_data2m,KL_documents,KL_topic_compo,assemblage_names_vect,documents,topic_compo,sum_data2m){
...
for (i_topic in 1:(nb_topics-1))
{
  for (j_topic in (i_topic+1):nb_topics)
  {
    ...
    for (rndzation in 1:nb_rndzations_best_real)
    {
      ...
      KL_topic_compo_randomized = KL_topic_compo[sample(seq(1,nrow_data2m,1),nrow_data2m),k1]
      ...
      KL_topic_comparison_MOTUwise_randomized[rndzation] = 1/2*(KL.plugin(KL_topic_compo[,k],KL_topic_compo_randomized) + KL.plugin(KL_topic_compo_randomized,KL_topic_compo[,k]))

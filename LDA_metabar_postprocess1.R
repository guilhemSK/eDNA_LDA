...
# Comparing topics to chemistery and lidar data, as well as comparing topics to each other within the best real:
abiotic_variables = 0
...
filled_with_gaps = 0
if (filled && data_pp && (length(which(colSums(data2m)==0))!=0))
{
  filled_with_gaps = 1
  Missing_positions_indices = vector(length=29*39,mode="numeric")
...
if (mpar)
{
  ...
  if (mnb_topics)
  {
    ...
    {
      local_subdirname = paste0(local_dirname,filename_insert,"_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_iter",nb_iter,"_nb_real",nb_real,remove_single_sites_insert,"/")
      cluster_subdirname = paste0(cluster_dirname,filename_insert,"_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_iter",nb_iter,"_nb_real",nb_real,remove_single_sites_insert,"/")
      filename = paste0(filename_insert,"_",alpha_insert,"_delta",delta,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_nb_iter",nb_iter,remove_single_sites_insert,".Rdata")  
    } else if (Rtopicmodels_VEM)
    {
      local_subdirname = paste0(local_dirname,filename_insert,"_",alpha_insert,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,remove_single_sites_insert,"/")
      cluster_subdirname = paste0(cluster_dirname,filename_insert,"_",alpha_insert,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,remove_single_sites_insert,"/")
      filename = paste0(filename_insert,"_",alpha_insert,"_nb_topics",nb_topics_range[1],"-",nb_topics_range[length(nb_topics_range)],"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,remove_single_sites_insert,".Rdata")
    }
...
for (par_index in 1:mpar_range)
{
  ...
  if (!mpar)
  {
    ...
    if (Rtopicmodels_Gibbs || Rtopicmodels_VEM)
    {
      ...
      } else if (best || best_keep)
      {
        if (best) 
        {
          if (Rtopicmodels_Gibbs)
          {
            local_subdirname = paste0(local_dirname,filename_insert,"_alpha",alpha,"_delta",delta,"_nb_topics",nb_topics,"_nb_iter",nb_iter,"_nb_real",nb_real,"_best",remove_single_sites_insert,"/")
            cluster_subdirname = paste0(cluster_dirname,filename_insert,"_alpha",alpha,"_delta",delta,"_nb_topics",nb_topics,"_nb_iter",nb_iter,"_nb_real",nb_real,"_best",remove_single_sites_insert,"/")
            filename = paste0(filename_insert,"_alpha",alpha,"_delta",delta,"_nb_topics",nb_topics,"_nb_real",nb_real,"_nb_iter",nb_iter,"_best",remove_single_sites_insert,".Rdata")  
          } else if (Rtopicmodels_VEM)
          {
            local_subdirname = paste0(local_dirname,filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best",remove_single_sites_insert,"/")
            cluster_subdirname = paste0(cluster_dirname,filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best",remove_single_sites_insert,"/")
            filename = paste0(filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best",remove_single_sites_insert,".Rdata")
          }
          if (!(file.exists(local_subdirname)))
            dir.create(local_subdirname)
          setwd(local_subdirname) 
        ...
        } else if (best_keep)
        {
          if (Rtopicmodels_Gibbs)
          {
            local_subdirname = paste0(local_dirname,filename_insert,"_alpha",alpha,"_delta",delta,"_topics",nb_topics,"_nb_iter",nb_iter,"_nb_real",nb_real,"_best_keep",remove_single_sites_insert,"/")
            cluster_subdirname = paste0(cluster_dirname,filename_insert,"_alpha",alpha,"_delta",delta,"_topics",nb_topics,"_nb_iter",nb_iter,"_nb_real",nb_real,"_best_keep",remove_single_sites_insert,"/")
            filename = paste0(filename_insert,"_alpha",alpha,"_delta",delta,"_nb_topics",nb_topics,"_nb_real",nb_real,"_nb_iter",nb_iter,"_best_keep",remove_single_sites_insert,".Rdata")
          } else if (Rtopicmodels_VEM)
          {
            local_subdirname = paste0(local_dirname,filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best_keep",remove_single_sites_insert,"/")
            cluster_subdirname = paste0(cluster_dirname,filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best_keep",remove_single_sites_insert,"/")
            filename = paste0(filename_insert,"_nb_topics",nb_topics,"_nb_real",nb_real,"_em_tol",em_tol,"_var_tol",var_tol,"_best_keep",remove_single_sites_insert,".Rdata")
          }
          if (!(file.exists(local_subdirname)))
            dir.create(local_subdirname)
          setwd(local_subdirname) 
    ...
    {
      local_subdirname = paste0(local_dirname,filename_insert,"hdp-faster_alpha",alpha,"_gamma",gamma,"_eta",eta,"_maxiter",maxiter,"_norep",remove_single_sites_insert)
      if (!(file.exists(local_subdirname)))
        dir.create(local_subdirname)
      setwd(local_subdirname) 
    ...
    if ((j_select == 1) && data_pp && !testdata && abiotic_variables)
    {
      ...
      for (i_topic in 1:(nb_topics-1))
      {
        for (j_topic in (i_topic+1):nb_topics)
        {
          ...
          KL_topic_comparison_samplewise[k,k1] = 1/2*(KL.plugin(KL_documents[,k],KL_documents[,k1]) + KL.plugin(KL_documents[,k1],KL_documents[,k]))
...
#         tmp.plot1 = qplot(x, y, data=spatial_topicmix_kriged[[k0]], geom="raster", fill=z.pred) +
#           scale_fill_gradientn(colours=c("white",terrain.color(nb_topics)[k])) +
#           theme_minimal() +
#           theme(legend.position="bottom", legend.text=element_text(size=7), 
#                 legend.title=element_text(size=8), axis.title=element_blank(), 
#                 axis.text = element_blank(),
...
#           guides(fill = guide_colorbar(barwidth = 8, barheight = 0.4, title.position="bottom"))
        ...
        } else if (data_gs)
        {
          ...
          #z.pred = rep(0,237488)
          tmp.plot[[k]] = tmp.plot[[k]] + geom_polygon(data=borders, aes(x=long*10, y=lat*10, group=group),
